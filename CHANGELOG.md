...
- Fix: The node snapshot that carries a new endpoint is sent before the `endpoint_added` event announcing it again, so a client that resolves the endpoint against its own node model knows it; the coalescing of `node_updated` had inverted that order
- Fix: A removed endpoint leaves the attribute cache when the removal is announced, and `get_node` awaits a cache rebuild that is already running, so a read after a structure change reports the structure the node has now
- Fix: A node is reported as a bridge when any endpoint carries the Aggregator device type, instead of endpoint 1 only, so bridges that place their aggregator elsewhere or nest one aggregator below another are flagged correctly
- Fix: (reported by lboue) Python client resolves each endpoint to its closest parent endpoint, so a device bridged by a nested Aggregator is recognized as a bridged device and reaches that Aggregator via the new `MatterNode.get_bridge_parent()`; `get_bridge_child_ids()` lists the devices an Aggregator bridges
- Fix: Python client resolves `MatterNode.get_compose_parent()` to the closest parent endpoint, which changes the answer for a composed device that reports its whole family in its PartsList, and resolves the relations again whenever the server reports an endpoint removal or a new node snapshot
- Fix: Python client tracks the device types and the endpoints of the last node snapshot instead of every device type and endpoint it ever saw, so an endpoint that stops being an Aggregator stops bridging the endpoints below it and an endpoint the node no longer reports stops taking part in the structure
- Fix: Dashboard resolves the endpoint tree of a node by the same rule as the Python client, so a bridge that lists the whole family below an aggregator and a device composed of further endpoints are shown with the same hierarchy in both
- Fix: Python client `MatterEndpoint.device_info` returns the endpoint's own `BridgedDeviceBasicInformation`, else the info of the composed device it is a part of, else the `BasicInformation` of the node. A bridged device that reports no info of its own now returns `None` and is logged once, instead of taking over the name and serial number of the bridge or of the Aggregator above it. `MatterNode.get_cluster()` returns `None` for an endpoint the node does not have, or no longer has, instead of raising `KeyError`
