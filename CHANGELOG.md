...
- Enhancement: `camera_get_capabilities` reports only facts the device states: sensor size, rate/distortion trade-off points, codecs, snapshot capabilities, `MaxNetworkBandwidth` and the current allocations, each allocated snapshot stream carrying the camera's own `hardware_encoder` flag so a client can see which stream holds an encoder. It derives no resolution list, because a hardcoded or derived list can offer a resolution the camera does not support, which is what issue #1054 reported
...
- Fix: `send_webrtc_provider_command` refuses a `ProvideOffer` or `SolicitOffer` payload that states both the `videoStreams` / `audioStreams` lists and the `videoStreamId` / `audioStreamId` they deprecate, with error 8. Against a camera at cluster revision 2 the server used to drop the singular ids, so `{"videoStreams": [5], "audioStreamId": null}` — video stream 5 plus auto-selected audio — opened a video-only session and reported success. A list sent to a camera below cluster revision 2, or to one whose revision the server has not read yet, is refused for the same reason unless it holds exactly one id, instead of being cut down to its first entry; an empty list is refused at every revision, since the field takes 1 to 16 entries
- Fix: The five camera commands and `send_webrtc_provider_command` refuse a message whose `args` is missing, `null` or not an object with error 8 naming the command. Such a message used to throw a `TypeError` inside the argument-key check, which the WebSocket route reported as error 0 (`UnknownError`), so a one-line request from any client reached an error code that names nothing. `send_webrtc_provider_command` now checks `node_id` and `endpoint_id` the way the five camera commands do, so a fractional `node_id` is error 8 rather than an uncaught conversion failure; a `node_id` sent as a string, which used to be converted by accident, is refused
- Fix: Every WebSocket command reads its arguments through one rule at the dispatch. A message that states no `args`, or states `null`, now means an empty argument set, so a command whose arguments are all optional answers it: `set_loglevel` and `get_vendor_names` used to report error 0 (`UnknownError`) for such a message, because the handler destructured `undefined`. A command with a required argument refuses the same message the way it refuses that argument being absent. An `args` that is a string, a number, a boolean or an array is refused with error 8 naming the command, where a handler used to read a string's indices as argument keys or throw. Handlers had begun writing `args ?? {}` and `args?.x` one at a time, which is the hole this closes for all of them. Two commands check the argument the empty set leaves absent rather than acting on it: `set_default_fabric_label` refuses a message that states no `label`, instead of renaming the fabric to the default label, persisting it and claiming the label for that connection, and `set_thread_dataset` names the argument it did not get instead of reporting the string `"undefined"` as an invalid dataset. A command that reads no arguments at all — `server_info`, `get_all_credentials`, `get_thread_border_routers`, `discover`, `get_loglevel`, `initiate_ota_upload` — now refuses a malformed `args` it used to ignore
...
- Enhancement: `camera_snapshot` captures from a snapshot stream the camera already has, whoever allocated it, whenever one meets the request's own bounds and is no smaller than the capability the call would otherwise allocate. Allocating a stream per call is what the cluster asks controllers to avoid, and each allocate competes for the encoders the livestream needs. A stream the call allocates and then answers with is left in place for the next one, whatever capability it came from; a call that fails gives back what it allocated. A stream allocated at a capability that requires the hardware encoder holds one of the camera's encoders while it exists, so on single-encoder hardware a later `camera_start_stream` fails with error 103 until the client releases it with `camera_release_stream`
- Enhancement: The `camera_snapshot` response always carries `stream_id`, naming the snapshot stream the camera still holds, which is the id `camera_release_stream` takes. The server never deallocates that stream on the success path: the device invoke cannot be aborted and the wait for it has to be bounded to keep the endpoint lock, so a give-back could be started but never awaited to an outcome the response can state, and the response would name a stream that is being removed. The field no longer carries `reused` or `allocated_by_server`: neither is anything the caller acts on, and `camera_get_capabilities` reports both
- Enhancement: `camera_snapshot` avoids the device's encoder when the camera has none free, clamping to a capability that does not require hardware encoding. Requesting a snapshot during an active livestream therefore no longer risks a `ResourceExhausted` device error. Free encoders are counted against the camera's own `MaxConcurrentEncoders`: a referenced video stream takes one, so one viewer on a camera that states four no longer costs picture size, and so does a snapshot stream the camera marks `HardwareEncoder`, which holds its encoder for as long as it exists whatever its reference count. When the camera still refuses, the next capability down is tried, and a remaining refusal is reported as error 102 or 103 rather than a raw device error
...
- Fix: `send_webrtc_provider_command` accepts the documented `ice_servers` shape. Its payload now reaches the same conversion `camera_start_stream` uses, so `{ urls, ... }` becomes the cluster's `ICEServerStruct` on both routes and an ICE server read from a `webrtc_callback` offer can be sent straight back. It used to be converted by field name only, which left the struct's mandatory `URLs` unset and a string where a list belongs, and the call failed inside matter.js's TLV encoder
- Enhancement: `send_webrtc_provider_command` checks every payload field against the cluster's own definition before the camera is asked — each number against the range its field encodes in, each string against the ceiling its field states, each list against the entry count its field takes, and each mandatory field for being present — and refuses a failure with error 8 naming the key as sent. Payloads that were accepted before and are now refused: a key naming no field of the command, a second key resolving to a field another already filled — matter.js dropped what it could not place and a duplicate overwrote silently, leaving the caller with a session it did not ask for — a payload omitting a mandatory field, a payload that is not an object, and an ICE server spelled in the cluster's `URLs` rather than the documented `urls`. The generic `device_command` route is unchanged and keeps taking every cluster's own field names
- Enhancement: `send_webrtc_provider_command` logs a warning when a `ProvideOffer` names an existing `webrtc_session_id` and also states `videoStreams`, `audioStreams`, `streamUsage` or `metadataEnabled` (in any spelling the payload's key matching accepts). The provider runs its whole stream-selection step only for a request whose `WebRTCSessionID` is null (spec § 11.5.6.3), so on a re-offer those fields change nothing and the response says nothing about them. The request is forwarded exactly as the caller wrote it — what such a field means is the camera's decision, not this server's. The gated fields are read from each field's own conformance in the Matter model, not listed in code; `originatingEndpointId` carries the same gate and is not among them, because the command drops it and injects the server's own
- Breaking (library only): `ControllerCommandHandler.sendWebRtcProviderCommand` is removed. It differed from `invokeWebRtcProviderCommand` only in converting a WebSocket payload first, and that conversion now happens at the WebSocket boundary where `camera_start_stream`'s arguments are parsed, so the class has one entry to a provider session and no wire shape reaches it. The `send_webrtc_provider_command` WebSocket command is unchanged; only a direct consumer of the `@matter-server/ws-controller` class is affected, and it calls `invokeWebRtcProviderCommand` with `fields` instead of `payload`, running the payload through `toProviderCommandFields`, which the package now exports for exactly that
...
- Enhancement: `send_webrtc_provider_command` also relays `ProvideIceCandidates`, so a client can trickle ICE candidates for a session through the checked argument boundary instead of only through the generic `device_command` in the cluster's own field names. A candidate a `webrtc_callback` `ice_candidates` event reports is sent back unchanged. The command answers `null` for it, because the cluster defines no response payload for it, and it establishes nothing: the originating endpoint, the stream-field reconciliation and the local session registration belong to `ProvideOffer` and `SolicitOffer`
- Fix: A `send_webrtc_provider_command` payload key is matched to a cluster field with case and the separators between words ignored. `camelize` read `webrtc_session_id` — the spelling every other command in this API uses for that id — as one word, and left the W3C `sdpMLineIndex` a `webrtc_callback` candidate carries unchanged where the cluster's field is `SDPMLineIndex`, so a client echoing back what the server had just handed it was refused for an unknown key. More spellings are accepted than before, never fewer, and stating one field twice under two of them is still refused
- Enhancement: `send_webrtc_provider_command` refuses an argument of its own that it does not take, with error 8, as its payload already refused a key naming no cluster field
- Breaking (library only): `ControllerCommandHandler.invokeWebRtcProviderCommand` takes `SessionEstablishingCommandName` — `ProvideOffer` or `SolicitOffer` — for its `commandName`, and the new `invokeProvideIceCandidates` carries the third command, which establishes no session and answers nothing. A consumer relaying a client's `command_name` splits on the exported `establishesWebRtcSession`; both it and the type are exported from `@matter-server/ws-controller`
- Enhancement: All five camera commands refuse an argument key they do not take, with error 8, instead of ignoring it — on `camera_start_stream` under a hint object as well. An `audio: { "min_bit_rate": 128000 }`, or a key beside the ones `camera_get_capabilities`, `camera_stop_stream` and `camera_release_stream` read, used to produce no error and no effect, which is the one thing these commands never do with a stated argument
- Fix: A codec's own level in the SDP offer is read as a decode ceiling instead of being ignored. `profile-level-id` (H.264) is mapped through ITU-T H.264 Table A-1 (MaxFS, MaxMBPS, MaxBR) and `level-id` (H.265) through ITU-T H.265 Table A.8 (MaxLumaPs) and Table A.9 (MaxLumaSr). An offer stating H.264 level 3.1 and no `max-fs` used to be treated as unconstrained and could be handed a 1080p stream the peer cannot decode. This changes what a common request returns: every browser offers H.264 at `profile-level-id=…1f` (level 3.1, MaxFS 3600 macroblocks) with no `max-fs`, so `camera_start_stream` with such an offer now allocates at most 1280x720 where it used to allocate 1920x1080. That is the resolution the peer stated it can decode; a client that wants more has to offer a higher level. A codec that states no level at all is unbounded by the offer, as before — the RFC-inferred default for an absent level is not applied, because H.264's is Baseline level 1 and inferring it would refuse offers no peer meant to restrict
- Fix: A codec's `a=fmtp` capability parameters are read under that codec's own payload format, never another's. H.264 states its frame size and rate as `max-fs` / `max-mbps` in macroblocks (RFC 6184 §8.1); H.265 states them as `max-lps` / `max-lsr` in luma samples and its frame rate as `max-fps` in frames per 100 seconds (RFC 7798 §7.1). Each overrides what the level states, which is what those parameters signal. Before this, H.264's names were read on an H.265 record, so `max-fs=1000000` on an H.265 payload type — a token H.265 does not define — lifted the ceiling `level-id` had stated by a factor of 260, and H.265's own `max-lps` and `max-lsr` were never read at all

- Fix: `camera_snapshot` keeps the encoder-using snapshot capabilities as fallbacks instead of removing them. With every encoder taken the server prefers the encoder-free capabilities, and it now tries the encoder-using ones the caller's own bounds allow after them, rather than ending the ladder at the last encoder-free rung. A camera that refused every encoder-free capability — realistic, since the encoder count the server reads comes from a subscription-backed cache that lags — failed the call with capabilities never tried. The fallback has a cost worth knowing: a snapshot stream allocated at an encoder-using capability is left in place like any other, so on a camera with one encoder it holds that encoder until a client sends `camera_release_stream`, and `camera_start_stream` fails with error 103 until then. `downgraded` is unchanged and still means the frame is smaller than the best capability the caller's own bounds allowed
- Fix: `camera_release_stream` lets the camera decide whether a stream can go. The deallocate now always goes out, and the camera's `INVALID_IN_STATE` is reported as error 104 (`CameraStreamInUse`), which is what the reference implementation answers for a reference count above 0 and for nothing else; the device's own error is kept as the cause. The reference count the server holds comes from a subscription-backed cache that can be behind the device in either direction: a stale zero used to let the refusal surface as error 0 (`UnknownError`) although 104 is what the API documents for it, and a stale non-zero refused the release outright, with no path left that reaches the device. The count now only fills in `reference_count` in the error details, and is absent when it is zero
- Fix: `camera_release_stream` drops its record of owning a stream when the camera answers `NOT_FOUND` for the deallocate, which is how the server's own give-back path already reads that answer. For a stream the camera never reported the record used to survive for the rest of the process run — nothing else drops it, since the reconciliation only drops a lease the camera reported and then stopped reporting — so `owned_by_server` kept naming a stream the camera does not have, and a reissued stream id could be matched to it
...
- Fix: The WebRTC provider's `ClusterRevision` is read from the endpoint's own cluster state instead of the attribute cache. The cache answers nothing until an attribute report has carried that attribute, so a `ProvideOffer` or `SolicitOffer` sent before one arrived was down-converted to the deprecated single `VideoStreamID`/`AudioStreamID` against a revision-2 camera, which drops every requested stream past the first
- Fix: A WebRTC session that outlives the server has a way back. `camera_get_capabilities` reports the camera's own `CurrentSessions` list, and `camera_stop_stream` ends any session on it, not only one this process run established. Before this an ungraceful restart left the camera holding a session nothing could name again: the server's tracking is in memory, `camera_stop_stream` answered `ended: false` for the id, and only `EndSession` decrements a stream's `ReferenceCount`, so `camera_release_stream` was refused with `InvalidInState` until the camera rebooted. On a single-encoder camera that is the whole device. The camera decides what an `EndSession` ends: it answers `NOT_FOUND` for any session whose fabric and peer node are not the caller's, which `established_by_this_server` reports in advance
...
- Fix: `camera_start_stream` translates each `ice_servers` entry into the cluster's `ICEServerStruct` instead of forwarding the object it received. The struct's field is `URLs` and is always a list (spec § 11.4.5.3), so the documented `{ "urls": "stun:..." }` entry reached the TLV encoder with the mandatory field unset and a string where a list belongs, and every call that carried ICE servers failed. An entry with an unknown key, no URL, or a value past the struct's own limits — 10 URLs of 2000 characters, a 508-character `username`, a 512-character `credential`, `caid` 0 to 65534 — is now refused with error 8 instead of failing inside the encoder. The `webrtc_callback` offer event reports its ICE servers under the same `urls` key, so what a client reads from an offer is what it can send back
- Fix: A camera session is owned by an identifier that is distinct for every connection the process accepts. It used to be the short connection tag the log lines carry, which wraps after 65536 connections; on a server up long enough to wrap, closing either holder of a reused tag ended both clients' camera sessions
- Fix: The TypeScript client library keeps bulk content out of the browser console. Every incoming frame and event is logged in full, and a `camera_snapshot` response carries the complete base64 frame, so each capture wrote hundreds of kilobytes of image data into the console — the same content the server already keeps out of its own log. A string longer than 1024 characters in an incoming frame or event is now logged as its length (`[699072 chars omitted]`), wherever it sits under that message — as a bare array entry too, which is how `attribute_updated` reports a value. Everything beside it stays: the codec, the resolution and the `stream_id` the snapshot response carries are still logged, and a WebRTC offer keeps its lines, because the ICE credentials in it are masked line by line ahead of this rule. The rule is by shape rather than by command name — a response states its `message_id` and not the command it answers — so nothing has to be kept in step with the server's list of commands whose response content it drops. An outgoing argument is unchanged and still logged whatever its length: it is the record of what the caller asked for, and an `import_test_node` dump is the whole record of a refused import. The Python client library is unchanged and still logs whole messages
- Fix: The connection tag the server's log lines carry is always four hex digits, so it forms a column that can be scanned; the first connections of a process, and the ones after each wrap, used to be written as `0` or `ff`
...
- Fix: The `a=ice-ufrag` and `a=ice-pwd` values inside an `sdp` are masked in the server's log and in the TypeScript client library's console output. Every request is written to the log in full and the offer was left whole, so the session's ICE credentials — which authenticate the peer's connectivity checks — reached the log on every `camera_start_stream` that carried an offer, and the camera's own reached a browser console through the `webrtc_callback` event. Only those two values are masked: the rest of the offer, the DTLS fingerprint included, is what the peer publishes to its counterpart and is what a failed session is read from. The client library's incoming frame and event logs masked nothing at all before this and now mask the same two values plus a TURN `username` and `credential` inside an ICE server, which is what a `webrtc_callback` offer carries from the camera. The Python client library is unchanged and still logs whole messages
- Fix: `camera_stop_stream` and `camera_release_stream` refuse a `webrtc_session_id` or `stream_id` above 65535 with error 8. `WebRTCSessionID`, `VideoStreamID`, `AudioStreamID` and `SnapshotStreamID` are uint16, so a larger value names no session or stream the camera can have, and `camera_stop_stream` answered `ended: false` for it — which reads as a session that had already ended
- Fix: `camera_snapshot` no longer writes the captured frame into the server's debug log. The response carries the complete base64-encoded image, and the mechanism that keeps a large response out of the log did not name the command. The codec, resolution, byte count and `downgraded` flag are logged on their own line, so what the server chose is still there.
...
- Fix: An `a=fmtp` parameter name is read whatever case it is written in. `a=fmtp` carries media type parameters, whose names are case-insensitive (RFC 6838 § 4.3), so an offer stating `MAX-FS` or `Max-Br` lost its decode ceiling and the peer could be handed a stream it cannot decode
