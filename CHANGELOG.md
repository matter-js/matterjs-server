...
- Enhancement: Stating any value under `audio` on `camera_start_stream` asks for audio. Such a caller is now told why it got none — no microphone, a codec narrowing that left nothing, or a device that refused the allocation — instead of receiving `audio: null`. A caller that stated no `audio` value still gets a video-only session
- Enhancement: When the camera has no capacity left, `camera_start_stream` deallocates an unreferenced stream to make room. `reference_count: 0` means allocated but with no listener, so freeing it costs nobody a live view. If nothing can be freed, the call can still succeed with a stream outside the range the server computed, reported as `degraded: true`, as long as that stream stays inside every bound the caller stated
...
- Enhancement: `camera_snapshot` gives its snapshot stream back before it answers, on success as well as on failure. A capability that requires the hardware encoder holds one of the camera's encoders for as long as its stream is allocated, so a client polling for stills used to leave one such stream behind per poll. On single-encoder hardware every call after the first then failed with error 103, and a client that disconnected without calling `camera_release_stream` left the streams on the device for good
- Enhancement: The `camera_snapshot` response no longer carries `stream_id`, `reused` or `allocated_by_server`. By the time a caller reads it there is normally no stream left to name. If the device refuses to take the stream back it stays server-owned, `camera_get_capabilities` reports it with `owned_by_server: true`, and `camera_release_stream` can free it
...
- Enhancement: `camera_start_stream` refuses a key a hint object does not take, with error 8, instead of ignoring it. An `audio: { "min_bit_rate": 128000 }` used to produce no error and no effect, which is the one thing these commands never do with a stated bound
...
- Fix: A stream deallocated to make room for a `camera_start_stream` that then never uses that capacity is re-allocated with the range, codec and stream usage it stated. The request may have failed, or succeeded by handing out a stream that was already there. Freeing therefore no longer costs another controller a stream it had pre-allocated. The device issues a new id, so the id itself is not restored
- Fix: A lease on a stream the camera never reports back is given up after five minutes, instead of being kept for the lifetime of the server. The per-endpoint lease list used to grow one entry per allocation for as long as the server ran on a camera whose `AllocatedVideoStreams` reports never arrive. A lease that outlived its stream could also be matched to a foreign stream the camera later issued the same id to, and `camera_release_stream` would then deallocate a stream this server does not own. That window is now five minutes rather than unbounded
- Fix: A WebRTC session the camera reports it no longer has (`NotFound`) is dropped by both places the camera commands track it in. The controller's own requestor used to keep its entry, which then named a session nothing could reach again and stayed for the lifetime of the server. The raw `device_command` path still leaves both trackings in place when the device answers `NotFound`
...
- Fix: Ending camera sessions and giving streams back is bounded to 10 seconds. A camera that stopped answering used to block server shutdown with no limit and hold that endpoint's command lock for as long as the transport kept retrying, which queued every other camera command behind it. The sessions of a release pass are ended concurrently, so one silent camera does not spend the budget the others need, and a session whose `EndSession` was abandoned stays tracked for the next pass
- Fix: A WebRTC callback carrying an endpoint id outside the valid range no longer aborts the shared event emit, which would have kept that signaling event from every WebSocket connection
...
- Enhancement: Adds error codes 102-106 (`CameraStreamIncompatible`, `CameraResourceExhausted`, `CameraStreamInUse`, `CameraStreamNotOwned`, `CameraNotSupported`) for the new camera commands, mirrored in the Websocket client constants and the Python client. Error 102's `bound.field` names the hint key as the wire spells it: `min_resolution`, `min_frame_rate`, `min_bit_rate`, `sample_rate` or `channel_count`. Error 103's `allocated` entries carry a `kind`, because the streams holding the capacity are not always the kind that was asked for — a refused snapshot reports the video streams, which are what hold the encoders
