...
- Enhancement: `camera_start_stream` refuses a key a hint object does not take, and `camera_snapshot` refuses an argument it does not take, both with error 8, instead of ignoring it. An `audio: { "min_bit_rate": 128000 }` used to produce no error and no effect, which is the one thing these commands never do with a stated bound
