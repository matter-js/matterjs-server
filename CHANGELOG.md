...
- Enhancement: `camera_snapshot` avoids the device's encoder while a video stream is live, clamping to a capability that does not require hardware encoding, so requesting a snapshot during an active livestream no longer risks a `ResourceExhausted` device error; when the camera still refuses, the next capability down is tried and any remaining refusal is reported as error 102 or 103 rather than a raw device error; `downgraded: true` reports that the frame is smaller than the one the caller's own bounds would otherwise have allowed, so a camera whose best capability needs no encoder anyway no longer reports a downgrade that did not happen; a camera that states no snapshot capability at all fails with `reason: "capability"` instead of `"bounds"`, so a client can tell "ask for less" from "this device cannot do it"
