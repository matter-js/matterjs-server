...
- Enhancement: `camera_snapshot` avoids the device's encoder while a video stream is live, clamping to a capability that does not require hardware encoding and reporting `downgraded: true`, so requesting a snapshot during an active livestream no longer risks a `ResourceExhausted` device error
