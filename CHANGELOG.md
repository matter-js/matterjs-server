...
- Enhancement: Adds Websocket commands `camera_get_capabilities`, `camera_start_stream`, `camera_stop_stream`, `camera_snapshot` and `camera_release_stream`, moving camera video/audio stream setup to the server: it computes the `VideoStreamAllocate` envelope, matches or allocates streams, and handles encoder exhaustion, instead of every consumer (dashboard, Home Assistant, …) reimplementing the same device traps; a session is tracked per node, endpoint and session id and ended on a single path, so a closing connection (including one that closes while the session is still being set up), a server shutdown, a peer-initiated end and a client's own `EndSession` on the raw path all leave the device's `ReferenceCount` correct instead of pinning the stream permanently
- Enhancement: `camera_get_capabilities` reports only device-stated facts (sensor size, rate/distortion trade-off points, codecs, snapshot capabilities, `MaxNetworkBandwidth`, current allocations), with no derived resolution list — a hardcoded or derived list can offer a resolution the camera does not support, which is what issue #1054 reported
- Enhancement: `camera_start_stream` reuses an already-allocated video or audio stream only when it is contained in the caller's requested range, not merely overlapping it, so a request for 1080p can no longer be served by a stream that may only deliver 720p (issue #1056); when every stream is in use, an unreferenced one is deallocated to make room, and if none can be freed the request can still succeed with a stream outside the server's own default range, reported as `degraded: true`, as long as it stays within any bounds the caller stated itself; a `camera_start_stream` or `camera_snapshot` that fails deallocates the streams it allocated for the attempt, which the caller could not release itself because it never receives their ids; every bound the caller stated is hard at every rung, whether the stream was freshly allocated, reused, or handed out degraded: the video codec list, the resolution, frame-rate and bit-rate bounds, the mandatory `stream_usage`, the audio codec list, `sample_rate`, `channel_count` and `bit_rate`, and a snapshot `max_resolution` ceiling are never widened, lowered, clamped or swapped to make a request fit — when nothing satisfies them the request fails with error 102; stating any `audio` value asks for audio, so a caller that stated one is told why it got none (no microphone, a codec narrowing that left nothing, a device that refused the allocation) instead of receiving `audio: null`, while a caller that stated none still gets a video-only session — a `camera_start_stream` bound the server rules out before asking the device names itself in `bound: { field, requested, limit }` — instead of returning a stream in a codec the caller cannot decode, a stream below the quality it asked for, or a snapshot larger than it can display; reuse also covers a stream the server allocated moments earlier that the camera's own `AllocatedVideoStreams` report has not named yet, so two `camera_start_stream` calls in quick succession no longer leave a duplicate stream behind that occupies an encoder slot and that nothing releases
- Enhancement: `camera_snapshot` gives its snapshot stream back before it answers, on success as well as on failure: a snapshot capability that requires the hardware encoder holds one of the camera's encoders for as long as the stream is allocated, so a client polling for stills previously left one such stream behind per poll — on single-encoder hardware every call after the first then failed with error 103, and a client that disconnected without calling `camera_release_stream` left them on the device for good. The response therefore no longer carries `stream_id`, `reused` or `allocated_by_server`: by the time a caller reads it there is normally no stream left to name. If the device refuses to take the stream back it stays server-owned, where `camera_get_capabilities` reports it with `owned_by_server: true` and `camera_release_stream` can free it
- Enhancement: `camera_snapshot` avoids the device's encoder when the camera has none free, clamping to a capability that does not require hardware encoding, so requesting a snapshot during an active livestream no longer risks a `ResourceExhausted` device error; how many are free is counted from the referenced video streams against the camera's own `MaxConcurrentEncoders`, so one viewer on a camera that states four no longer costs picture size; when the camera still refuses, the next capability down is tried and any remaining refusal is reported as error 102 or 103 rather than a raw device error; `downgraded: true` reports that the frame is smaller than the one the caller's own bounds would otherwise have allowed, so a camera whose best capability needs no encoder anyway, and a camera with more encoders than are in use, no longer report a downgrade that did not happen; a camera that states no snapshot capability at all fails with `reason: "capability"` instead of `"bounds"`, so a client can tell "ask for less" from "this device cannot do it"
- Enhancement: Introduces Websocket Schema version 14 (backward compatible): the five camera commands are detectable with `server_info.schema_version >= 14`, and every codec (`H264`, `H265`, `H266`, `AV1`, `OPUS`, `AAC`, `JPEG`, `HEIC`) and stream usage (`Recording`, `Analysis`, `LiveView`) on them is a name in both directions, so what `camera_get_capabilities` reports can be sent straight back as a `camera_start_stream` hint, `stream_usage` or a `camera_snapshot` codec; `two_way_talk_support` is a name as well (`NotSupported`, `HalfDuplex`, `FullDuplex`). A camera that states the device-only `Internal` usage reports it too, and `camera_start_stream` refuses it
- Fix: A `camera_start_stream` whose connection closes while the camera is still answering `ProvideOffer` now gives back the video and audio streams it allocated, instead of ending the session and leaving them on the device where nothing could reach them — the caller never received their ids, so `camera_release_stream` could not free them either
- Fix: A stream deallocated to make room for a `camera_start_stream` that then never uses that capacity — the request failed, or succeeded by handing out a stream that was already there — is re-allocated with the range, codec and stream usage it stated, so freeing no longer costs another controller a stream it had pre-allocated for nothing (the device issues a new id, so the id itself is not restored)
- Fix: A lease on a stream the camera never reports back is given up after five minutes instead of being kept for the lifetime of the server: the per-endpoint lease list no longer grows one entry per allocation for as long as the server runs on a camera whose `AllocatedVideoStreams` reports never arrive, and a lease that outlived its stream can only be mistaken for a foreign stream the camera later issues the same id to within those five minutes rather than indefinitely — that mistake would let `camera_release_stream` deallocate a stream this server does not own
- Fix: A command that reaches the controller before it finished starting now fails with the SDK stack error code, as every other controller failure does, instead of a generic error carrying no code
- Enhancement: Adds error codes 102-106 (`CameraStreamIncompatible`, `CameraResourceExhausted`, `CameraStreamInUse`, `CameraStreamNotOwned`, `CameraNotSupported`) for the new camera commands, mirrored in the Websocket client constants and the Python client
