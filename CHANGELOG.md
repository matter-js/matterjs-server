...
- Fix: `get_thread_diagnostics` without an `ext_pan_id` no longer returns a complete diagnostics batch that is past the server cache TTL, so a client cannot act on Thread evidence the server would refuse to serve for a single network. Batches reporting a failed query (`partialReason`) are still returned at any age, because the panel that explains the failure shows nothing without them
