...
- Fix: `get_thread_diagnostics` without an `ext_pan_id` no longer returns a diagnostics batch that is past the server cache TTL, so a client cannot act on Thread evidence the server would refuse to serve for a single network. This covers the snapshots of a query still in flight (`in_progress`, `meshcop_no_responses_yet`, `rest_no_responses_yet`), which carry nodes. A batch that reports a query ending without data — `timeout`, `dtls_failed`, `border_router_unreachable` and the other terminal reasons — is still returned at any age, because the panel that explains the failure shows nothing without it
