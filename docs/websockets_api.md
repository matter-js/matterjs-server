...
`allocated` lists what is on the camera now, per kind. `reference_count` is the device's own count of listeners, and `owned_by_server` says this server allocated the stream, which is what `camera_release_stream` requires.
...
| `ice_servers` | array of objects, optional | Passed to the WebRTC session setup unchanged |
...
`video` takes **ranges**: `codecs`, `min_resolution`, `max_resolution`, `min_frame_rate`, `max_frame_rate`, `min_bit_rate`, `max_bit_rate`. `audio` takes **exact values**, not ranges: `codecs`, `channel_count`, `sample_rate`, `bit_rate`. Any other key under either object is refused with error 8.

Every bound the caller states is hard in both directions: a codec list, floor or ceiling that nothing satisfies fails with error 102 rather than returning something else. Setting `min_resolution == max_resolution` pins an exact value, and takes that capacity from every other client sharing the camera (spec §15.2.1.2.2), so leave a bound unset unless an exact value is required. Stating any value under `audio` asks for audio. A caller that asks for audio and gets none normally sees a typed failure: error 102 when no capability, codec or range fits, error 103 when the camera has no encoder capacity left for it, or error 7 (`SDKStackError`) when the device accepts the allocation but never returns a stream id. A device status the allocation ladder does not recognize — anything other than `DynamicConstraintError`, `ResourceExhausted` or `ConstraintError` — is not translated into one of those codes and surfaces as error 0 (`UnknownError`) instead. Stating none leaves the track to the server, and `audio` in the response is `null` when no audio stream can be resolved.

A request that leaves nothing for the offer to carry fails with error 102, `reason: "capability"`, `device` and `requested` both empty, rather than sending an offer with neither track. That happens when `video: false` and `audio: false` are both stated, or when `video: false` is stated and no audio stream could be resolved for a caller that left `audio` to the server — no microphone, no codec match, or a refused allocate all end there the same way. Asking for at least one track succeeds with everything else about the request unchanged.

Response: `{ webrtc_session_id, mode, video, audio }`. `mode` is `"provide_offer"` or `"solicit_offer"`. `video` and `audio` are `null` when the track was left out or, for `audio`, when no stream was resolved for a caller that asked for none.

| Field | On | Meaning |
|---|---|---|
| `stream_id` | both | The id `camera_release_stream` takes for this track's `kind`. A stream this call allocated can also be reached later through `camera_get_capabilities`'s matching `video_stream_id` / `audio_stream_id`, once it is listed there with `owned_by_server: true` |
...
| `allocated_by_server` | both | This server allocated the stream, so `camera_release_stream` can free it |
...
`kind` is `"video"`, `"audio"` or `"snapshot"`. `stream_id` is the `stream_id` a `camera_start_stream` response carried for that track, or a `video_stream_id` / `audio_stream_id` / `snapshot_stream_id` from `camera_get_capabilities` with `owned_by_server: true`.
