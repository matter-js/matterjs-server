...
- **Five new commands** — `camera_get_capabilities`, `camera_start_stream`, `camera_stop_stream`, `camera_snapshot`, `camera_release_stream` — move Matter camera stream setup into the server: it computes the `VideoStreamAllocate` envelope, reuses or allocates video/audio streams, reacts to the device's rejections, tracks the WebRTC session per node/endpoint/session id and ends it when the connection closes. Clients detect them via `server_info.schema_version >= 14`. The raw `send_webrtc_provider_command` / `device_command` path is unchanged, so a client doing its own allocation keeps working.
  - `camera_get_capabilities` reports only device-stated facts: `video` (sensor, `rate_distortion_points`, codecs), `audio`, `snapshot.capabilities`, `limits` (including `max_network_bandwidth`, the bit-rate ceiling the server caps a stream at) and the `allocated` streams per kind with `reference_count` and `owned_by_server`. There is deliberately **no resolution list** — the camera does not state one, and a derived list offers resolutions it cannot serve.
  - `camera_start_stream` takes range hints (`codecs`, `min_*` / `max_*` for resolution, frame rate and bit rate) and answers with the allocated envelope per track. Every bound the caller states is hard in both directions: nothing is widened, lowered or swapped for another codec to make a request fit.
  - `camera_snapshot` returns base64 image bytes plus the capability it used and `downgraded`, which says the frame is smaller than the best capability the request's own bounds allowed.
- **Codecs and stream usages are names, not numbers, in both directions.** `H264`, `H265`, `H266`, `AV1`, `OPUS`, `AAC`, `JPEG`, `HEIC` — the SDP spelling of the cluster enum — on `video.codecs`, `rate_distortion_points[].codec`, `audio.codecs`, `image_codec`, the allocated streams' `video_codec` / `audio_codec` / `image_codec`, a started stream's `codec`, a snapshot's `codec` and the `device` / `requested` lists on error 102. Hints and `camera_snapshot`'s `codec` take the same names, case-insensitively, so a client can send back what it read. `bound` on error 102 names a caller bound `camera_start_stream` ruled out before asking the device; the snapshot path reports `reason: "bounds"` without it. `two_way_talk_support` is a name as well (`NotSupported`, `HalfDuplex`, `FullDuplex`). Stream usages are names on the same rule: `supported_stream_usages`, `stream_usage_priorities` and an allocated stream's `stream_usage` report `Recording` / `Analysis` / `LiveView`, the spelling `camera_start_stream` already took (`Internal` is device-only and refused). A codec value the cluster enum does not define is reported as its decimal digits and accepted back in that spelling, because a camera may use a codec this build does not name. The stream usages are a closed set: a value the enum does not define is reported by number and cannot be requested.
- **New error codes 102-106** — `CameraStreamIncompatible`, `CameraResourceExhausted`, `CameraStreamInUse`, `CameraStreamNotOwned`, `CameraNotSupported` (OHF extensions; python-matter-server codes stop at 11). Error 102 carries `reason: "codec" | "bounds" | "capability"`: `capability` means the device states no capability of the kind the command needs, so unlike the other two no different request can succeed. It also carries `bound: { field, requested, limit }` when the server ruled a caller bound out before asking the device, and `device_status` when a device rejection produced it.
