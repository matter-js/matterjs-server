...
- **New error codes 102-106** — `CameraStreamIncompatible`, `CameraResourceExhausted`, `CameraStreamInUse`, `CameraStreamNotOwned`, `CameraNotSupported` (OHF extensions; python-matter-server codes stop at 11). Error 102 carries `reason: "codec" | "bounds" | "capability"`: `capability` means the device states no capability of the kind the command needs, so unlike the other two no different request can succeed. It also carries `bound: { field, requested, limit }` when the server ruled a caller bound out before asking the device, and `device_status` when a device rejection produced it. `bound.field` is the hint key as the wire spells it: `min_resolution`, `min_frame_rate`, `min_bit_rate`, `sample_rate` or `channel_count`. Error 103's `allocated` entries carry a `kind`, because the streams holding the capacity are not always the kind that was asked for. Error 106's `missing_clusters` names the cluster ids the endpoint does not expose, and the error is raised when either one is missing.
