...
- **New command `get_thread_diagnostics`.** Args: optional `ext_pan_id` (single network) and `force` (bypass cache).
  ...
  - Without `ext_pan_id`: returns the **current cache** for all known networks (an array, possibly empty) **immediately**, and kicks off a background refresh (honoring `force`) whose fresh batches arrive via `thread_diagnostics_updated`. It does not wait for the refresh — use the `ext_pan_id` form when you need synchronously-fresh data for one network. A complete batch older than the server's cache TTL is not returned, so a caller never receives evidence the server itself would no longer serve as a cache hit; a batch whose `partialReason` reports a query that ended without data (`timeout`, `dtls_failed`, `border_router_unreachable`, `petition_rejected`, `no_credentials`, `no_source`, `rest_unreachable`, `rest_protocol`) is returned regardless of age, because it explains a failure rather than describing a network. The snapshots of a query still in flight (`in_progress`, `meshcop_no_responses_yet`, `rest_no_responses_yet`) carry nodes and expire with complete batches.
