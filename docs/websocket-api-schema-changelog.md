...
- **Five new commands** — `camera_get_capabilities`, `camera_start_stream`, `camera_stop_stream`, `camera_snapshot`, `camera_release_stream` — move Matter camera stream setup into the server. It computes the `VideoStreamAllocate` envelope, reuses or allocates video and audio streams, reacts to the device's rejections, and tracks the WebRTC session per node, endpoint and session id, ending it when the connection closes. Clients detect the commands via `server_info.schema_version >= 14`. The raw `send_webrtc_provider_command` / `device_command` path is unchanged, so a client doing its own allocation keeps working.
...
- **`camera_snapshot`** takes optional `max_resolution` and `codec`; any other argument is refused with error 8, the same as an unknown hint key on `camera_start_stream`. It returns `{ data, codec, resolution, downgraded }`. `data` is base64 image bytes and `downgraded` says the frame is smaller than the best capability the request's own bounds allowed. The response names no stream: the snapshot stream is deallocated before the answer is sent, because a capability that needs the hardware encoder holds one of the camera's encoders while its stream exists.
- **`camera_release_stream`** takes `kind` (`"video"`, `"audio"` or `"snapshot"`) and `stream_id`.
...
- **New error codes 102-106** — `CameraStreamIncompatible`, `CameraResourceExhausted`, `CameraStreamInUse`, `CameraStreamNotOwned`, `CameraNotSupported` (OHF extensions; python-matter-server codes stop at 11). Error 102 carries `reason: "codec" | "bounds" | "capability"` and, for a failure about one track, `track: "video" | "audio"`. `capability` covers two different cases, which `track` tells apart: a track the caller asked for cannot exist — the device states no capability of that kind, or the offer rejects that media section, and no different request can succeed — and a `camera_start_stream` request that leaves both tracks out entirely, reported with no `track` and `device`/`requested` both empty, which is fixed by asking for at least one track. It also carries `bound: { field, requested, limit }` when the server ruled a caller bound out before asking the device, and `device_status` when a device rejection produced it. `bound.field` is the hint key as the wire spells it: `min_resolution`, `min_frame_rate`, `min_bit_rate`, `sample_rate` or `channel_count`. Error 103's `allocated` entries carry a `kind`, because the streams holding the capacity are not always the kind that was asked for. Error 106's `missing_clusters` names the cluster ids the endpoint does not expose. `camera_start_stream` raises 106 when either cluster is missing; `camera_get_capabilities`, `camera_snapshot` and `camera_release_stream` raise it only when the AV Stream Management cluster itself is missing.
