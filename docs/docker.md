...
| `LISTEN_ADDRESS`      | IP address to bind WebSocket server                  | (all interfaces)     | Single IP address (use CLI for multiple). A network interface can be defined using `{{ifname}}` |
