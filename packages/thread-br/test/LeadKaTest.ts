...
describe("LeadKa", () => {
    describe("buildRequest", () => {
        it("encodes the session ID as a 2-byte BE TLV", () => {
            const payload = LeadKa.buildRequest(0x0007);
            const entries = BasicTlv.walk(payload);
            expect(entries).to.have.lengthOf(1);
            expect(entries[0].type).to.equal(MeshCopTlvType.COMMISSIONER_SESSION_ID);
            expect(entries[0].value).to.deep.equal(new Uint8Array([0x00, 0x07]));
        });

        it("encodes max uint16 session ID correctly", () => {
            const payload = LeadKa.buildRequest(0xffff);
            const entries = BasicTlv.walk(payload);
            expect(entries[0].value).to.deep.equal(new Uint8Array([0xff, 0xff]));
        });

        it("round-trips session ID 0 (edge case)", () => {
            const payload = LeadKa.buildRequest(0);
            const entries = BasicTlv.walk(payload);
            expect(entries[0].value).to.deep.equal(new Uint8Array([0x00, 0x00]));
        });
    });
    ...
    describe("round-trip", () => {
        it("encodes session ID and can verify it in the raw TLV bytes", () => {
            ...
            const entries = BasicTlv.walk(payload);
            const decoded = (entries[0].value[0] << 8) | entries[0].value[1];
