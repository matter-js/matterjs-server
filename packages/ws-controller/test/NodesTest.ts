...
describe("Nodes", () => {
    describe("isNodeAvailable", () => {
        ...
        it("returns true when Connected even if debounce pending", () => {
            expect(nodes.isNodeAvailable(NodeConnectionState.Connected, true)).to.equal(true);
        });

        it("returns true when Reconnecting with debounce pending", () => {
            expect(nodes.isNodeAvailable(NodeConnectionState.Reconnecting, true)).to.equal(true);
        });

        it("returns true when WaitingForDeviceDiscovery with debounce pending", () => {
            expect(nodes.isNodeAvailable(NodeConnectionState.WaitingForDeviceDiscovery, true)).to.equal(true);
        });

        it("returns true when Disconnected with debounce pending", () => {
            expect(nodes.isNodeAvailable(NodeConnectionState.Disconnected, true)).to.equal(true);
        });

        it("returns false when Reconnecting without debounce", () => {
            expect(nodes.isNodeAvailable(NodeConnectionState.Reconnecting)).to.equal(false);
        });
    ...
    describe("processStateChange", () => {
        ...
        it("reports unavailable when Connected -> Disconnected (no debounce)", () => {
            ...
            const result = nodes.processStateChange(TEST_NODE_ID, NodeConnectionState.Disconnected, false);
        ...
        it("reports unavailable when Connected -> WaitingForDeviceDiscovery (no debounce)", () => {
            nodes.seedState(TEST_NODE_ID, NodeConnectionState.Connected);
            const result = nodes.processStateChange(TEST_NODE_ID, NodeConnectionState.WaitingForDeviceDiscovery, false);
            expect(result.availabilityChanged).to.equal(true);
            if (result.availabilityChanged) {
                expect(result.available).to.equal(false);
            }
        ...
        it("reports available when Disconnected -> Connected", () => {
            ...
            const result = nodes.processStateChange(TEST_NODE_ID, NodeConnectionState.Connected, false);
            expect(result.availabilityChanged).to.equal(true);
            if (result.availabilityChanged) {
        ...
        it("does NOT report change when Connected -> Reconnecting with debounce pending", () => {
            nodes.seedState(TEST_NODE_ID, NodeConnectionState.Connected);
            const result = nodes.processStateChange(TEST_NODE_ID, NodeConnectionState.Reconnecting, true);
        ...
        it("does NOT report change when Reconnecting -> WaitingForDeviceDiscovery with debounce pending", () => {
            nodes.seedState(TEST_NODE_ID, NodeConnectionState.Connected);
            nodes.processStateChange(TEST_NODE_ID, NodeConnectionState.Reconnecting, true);
            const result = nodes.processStateChange(TEST_NODE_ID, NodeConnectionState.WaitingForDeviceDiscovery, true);
            expect(result.availabilityChanged).to.equal(false);
            expect(nodes.isAvailable(TEST_NODE_ID)).to.equal(true);
        });
        ...
        it("Connected -> Reconnecting -> WaitingForDeviceDiscovery stays available while timer armed", () => {
            ...
            const r1 = nodes.processStateChange(TEST_NODE_ID, NodeConnectionState.Reconnecting, true);
            expect(r1.availabilityChanged).to.equal(false);
            expect(nodes.isAvailable(TEST_NODE_ID)).to.equal(true);

            // Reconnecting -> WaitingForDeviceDiscovery (timer still armed): still available
            const r2 = nodes.processStateChange(TEST_NODE_ID, NodeConnectionState.WaitingForDeviceDiscovery, true);
            expect(r2.availabilityChanged).to.equal(false);
            expect(nodes.isAvailable(TEST_NODE_ID)).to.equal(true);

            // WaitingForDeviceDiscovery -> Connected: timer cleared, stays available
            const r3 = nodes.processStateChange(TEST_NODE_ID, NodeConnectionState.Connected, false);
        ...
        it("full lifecycle: Connected -> Reconnecting -> Disconnected -> Connected", () => {
            ...
            const r2 = nodes.processStateChange(TEST_NODE_ID, NodeConnectionState.Disconnected, true);
            expect(r2.availabilityChanged).to.equal(false);
            expect(nodes.isAvailable(TEST_NODE_ID)).to.equal(true);
