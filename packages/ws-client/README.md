...
`stream_id` is what `camera_release_stream` takes for that track's `kind`; it is the only way to reach that command for a stream this call allocated. `reused` says the stream was already on the camera, and `allocated_by_server` says this server allocated it, so `camera_release_stream` can free it. `degraded` is present and `true` only on `video`, for a stream that does not fit the range the server computed while still meeting every bound the caller stated; it is absent otherwise.
