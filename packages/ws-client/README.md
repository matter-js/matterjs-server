...
Force-deallocates a stream the server owns, so the next request allocates fresh instead of reusing it. `kind: "snapshot"` only ever names a stream a `camera_snapshot` allocated and the device then refused to take back; every other snapshot stream fails with `CAMERA_STREAM_NOT_OWNED_ERROR_CODE`.
